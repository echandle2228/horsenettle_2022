...
NGerm.m <- aov(germ ~ Treatment, data = Germ)
...
ggplot(Germ, aes(x = Treatment, y = germ, fill = Treatment)) +
  geom_boxplot(alpha = 0.8) +
...
Pdiam.m <- aov(PD_mean ~ Region + Population, data = Control)
summary(Pdiam.m)  
Pdiam.m <- lmer(PD_mean ~ Region + (1|Population), data = Control)

(Pdiam.m.a <- anova(Pdiam.m))
(Pdiam.m.r <- ranova(Pdiam.m))


ggplot(Control, aes(x = Region, y = PD_mean, fill = Region)) +
  geom_boxplot(alpha = 0.8) +
...
NPdiam.m <- lmer(PD_mean ~ Treatment + (1|Population), data = Pdiam)
