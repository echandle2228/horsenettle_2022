...
ggplot(morph, aes(Block_2, style_length, fill = Block_2)) +
  geom_boxplot()
...
```{r}
pistil.m <- lmer(style_length ~ Treatment * Region + Population + Block_2 +(1|ID) + Population, data = morph)
...
emmeans(pistil.m, specs=pairwise ~ Treatment*Region, adjust="tukey")
...
stamen.m <- lmer(stamen_length ~ Treatment*Region + Population + Block_2 + (1|ID), data = morph)
...
# Post hoc test for significant interactions
emmeans(stamen.m, specs=pairwise ~ Treatment*Region, adjust="tukey")
...
ovule.m <- glmer(ovule_number ~ Treatment * Region + Population + Block_2 + (1|ID), data = morph, family = "poisson")

Anova(ovule.m)
summary(ovule.m) 

# Post hoc test for significant interactions
emmeans(ovule.m, specs=pairwise ~ Treatment*Region, adjust="tukey")
...
ggplot(Iden_Mean, aes(x = style_length, y = stamen_length, color = Treatment, linetype = Treatment)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE) +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "*`,`~")),
           label.x.npc = "left", label.y.npc = "top")+
  HStheme() +
  scale_linetype_manual(values = c("Heat" ="dashed", "Control" = "solid"))+
  scale_color_manual(values = c("#474747", "#FF3030")) +
  scale_y_continuous(limits = c(5,9)) +
  labs(x = "Mean Female Structure Length (mm)", y = "Mean Male Structure Length (mm)")
